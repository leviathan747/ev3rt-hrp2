...
static const Escher_ClassNumber_t Navigation_task_numbers[ Navigation_STATE_MODELS ] = {
  Navigation_TASK_NUMBERS
};
static const Escher_ClassNumber_t * const class_thread_assignment[ SYSTEM_DOMAIN_COUNT ] = {
  &Navigation_task_numbers[0]
};
...
typedef union {
  ...
  Navigation_DomainEvents_u mc_events_in_domain_Navigation;
...
Escher_SendSelfEvent( Escher_xtUMLEvent_t * event )
{
  ...
  Escher_mutex_lock( SEMAPHORE_FLAVOR_SQUEUE );
  if ( q->tail == 0 ) {
    q->head = event;
  } else {
    q->tail->next = event;
  ...
  q->tail = event;
  Escher_mutex_unlock( SEMAPHORE_FLAVOR_SQUEUE );
  Escher_nonbusy_wake( t );
}
 ...
 * event will be returned.
 */
static Escher_xtUMLEvent_t * DequeueOoaSelfEvent( const u1_t );
static Escher_xtUMLEvent_t * DequeueOoaSelfEvent( const u1_t t )
{
  Escher_xtUMLEvent_t * event;
  xtUMLEventQueue_t * q = &self_event_queue[ t ];
  Escher_mutex_lock( SEMAPHORE_FLAVOR_SQUEUE );
